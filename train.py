...
from rms_measure import calculate_rms_from_fits 
import subprocess
    ...
    for line in msg.split(os.linesep):
        if (line.find('Peak value') > -1):
            ...
            if '+/-' in line:
                ...
                pa = float(fds[-1])
                err_pa = 0.0
    return peak_flux, err_peak_flux, int_flux, err_int_flux, ra, dec, major, err_major, minor, err_minor,pa, err_pa

def do_predict_batch(pred_func, input_dir, mir_dir, source_founding=0):
    ...
    if n_tasks >1 :
        ...
        myrank = task_index #int(rankinfo[0]) - 1 #slurm array job id starting from 1?
        total_proc = n_tasks #int(rankinfo[1])
        slurm_id =  int( os.environ['SLURM_JOB_ID'] )#int(rankinfo[2])
    ...
    for fn in file_nms:
        if not fn.endswith('.png'):
            continue
        if source_founding==0:
           input_file = os.path.join(input_dir, fn)
           img = cv2.imread(input_file, cv2.IMREAD_COLOR)
           results = predict_image(img, pred_func)
           fits_file = os.path.splitext(fn)[0] + ".fits" #fn.split('.')[0] + ".fits"
           hdu = fits.open(os.path.join(input_dir, fits_file))[0]
           w = wcs.WCS(hdu.header).celestial
           #bane_comd = 'BANE --cores=%d --compress %s' % (20, os.path.join(input_dir, fits_file))
           #os.system(bane_comd)
           rms_file = os.path.splitext(fn)[0] + "_rms.fits"           
           local_rms = calculate_rms_from_fits(middle=True, mean=True, \
                                   boxsize=18, filename=os.path.join(input_dir,rms_file)) 
           med = np.nanmedian(hdu.data)
           mad = np.nanmedian(np.abs(hdu.data - med))
           local_sigma = mad / np.sqrt(2) * erfinv(2 * 0.75 - 1)
           for r in results:
               #format: xmin-ymin-xmax-ymax
               box_str = '{:.5f}-{:.5f}-{:.5f}-{:.5f}'.format(*list(r.box))
               x1 = float(box_str.split('-')[0])
               y1 = float(box_str.split('-')[1])
               x2 = float(box_str.split('-')[2])
               y2 = float(box_str.split('-')[3])
               ...
               y2 = int(y2)
               #debug
               #logger.info("shape:(%s,%s),x1: %s,x2: %s,y1: %s,y2: %s" % (str(hdu.data.shape[1]),str(hdu.data.shape[0]),str(x1),str(x2),str(y1),str(y2)))
               #logger.info(fn)
               if cfg.DATA.CLASS_NAMES[r.class_id] == 'cs':
                  med = np.nanmedian(hdu.data)
                  clip_level = med + 3 * local_sigma
                  mir_file = mir_dir +"/" + os.path.splitext(fn)[0] + ".mir"
                  ...
                  err_pa = derive_miriad_from_msg(msg)
                  #filename, labelname, score, box, local_rms, peak_flux, err_peak_flux, int_flux, err_int_flux, ra, 
                  #dec, major, err_major, minor, err_minor, pa, err_pa
                  #logger.info(fn, cfg.DATA.CLASS_NAMES[r.class_id], \
                  #r.score, box_str, local_rms, peak_flux, err_peak_flux, int_flux, err_int_flux, ra, \
                  #dec, major, err_major, minor, err_minor, pa, err_pa)
                  if peak_flux == [] :
                     ...
                     #peak = np.max(box_data)
                     #peak_xy = np.where(hdu.data==np.max(box_data))
                     #peak_ra, peak_dec = w.all_pix2world(peak_xy[1][0], peak_xy[0][0],1)
                     #if peak_ra < 0 :
                     #   peak_ra = peak_ra + 360
                     ##image file name,label,score,box,peak(Jy),peak_x,peak_y,peak_ra,peak_dec
                     #tags.append(
                  #"{},{},{:.2f},{},{:.5f},{},{},{:.5f},{:.5f}".format(fn, cfg.DATA.CLASS_NAMES[r.class_id], r.score, box_str, peak, peak_xy[1][0], peak_xy[0][0], peak_ra, peak_dec))
                  
                  else:
                     tags.append(
                     "{},{},{:.2f},{},{:.5f},{:.5f},{:.5f},{:.5f},{:.5f},{},{},{:.5f},{:.5f},{:.5f},{:.5f},{:.5f},{:.5f}".format(\
                     fn, cfg.DATA.CLASS_NAMES[r.class_id], \
                     r.score, box_str, local_rms, peak_flux, err_peak_flux, int_flux, err_int_flux, ra, \
                     dec, major, err_major, minor, err_minor, pa, err_pa))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sourcefounding', help='Souce founding enable, 0: do, 1: no do', default=0)

    parser.add_argument('--mirdir', help="miriad images dir corresponding for pred images")
    ...
    if args.visualize or args.evaluate or args.predict or args.batchpred:
        ...
        if args.visualize:
            ...
            if args.predict:
                ...
                do_predict_batch(OfflinePredictor(predcfg), args.batchpred, args.mirdir, int(args.sourcefounding))
            elif args.evaluate:
        ...
        #train_dataflow = get_train_dataflow(rotate=False) #already augmented 

...
__all__ = ['interactive_imshow',
           ...
           'draw_boxes']
# BGR color
# red, black, Magenta, green
colors_BGR = {'cs': (0,0,255), 'fr1': (0,0,0), 'fr2': (255,0,255), 'core_jet': (0,128,0)}
...
def draw_text(img, pos, text, color, font_scale=0.4):
    ...
    # orig color (222,222,222)
    cv2.putText(img, text, text_bottomleft, font, font_scale, (222, 222, 222), lineType=cv2.LINE_AA)
    return img


def draw_boxes(im, boxes, labels=None, color=None, Source_finding=False):
    """
    Args:
    ...
    for i in sorted_inds:
        ...
        if Source_finding :
           cv2.rectangle(im, (box[0], box[1]), (box[2], box[3]),
                      color=colors_BGR[labels[i].split(',')[0]], thickness=1)
        else :
           if labels is not None:
              im = draw_text(im, (box[0], box[1]), labels[i], color=colors_BGR[labels[i].split(',')[0]])
           cv2.rectangle(im, (box[0], box[1]), (box[2], box[3]),
                      color=colors_BGR[labels[i].split(',')[0]], thickness=1)
    return im
